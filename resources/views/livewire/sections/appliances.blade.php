@if ($sectiondata != null)
    <section class="bg-top bg-no-repeat w-full bg-lightblue dark:bg-slate-950 py-8 bg-cover" style="background-image: url('{{ asset('/images/boxbackground.svg') }}');">
        <div class="mx-auto container lg:px-0 px-10 pb-10">
            <div class="flex flex-wrap justify-center items-center -mx-4">
                ...
                <div class="w-full md:w-6/12 px-4 mb-4 md:mb-0">
                    <div class="text-3xl font-serif font-medium text-primary dark:text-white lg:text-5xl my-6 lg:py-0 leading-8 w-full max-w-xl">
                        {{$sectiondata->title}}
                    </div>
                </div>
                <!-- Text Column -->
            ...
            <div class="grid mt-8 grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                ...
                @foreach ($appliances as $appliance)
                    <a href={{$appliance->button_url}} class="border-2 rounded-lg border-sky-200 dark:border-black bg-gray-50 dark:bg-slate-900 p-6 text-center transition-transform transform hover:scale-105 hover:shadow-lg">
